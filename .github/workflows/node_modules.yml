...
jobs:
  check:
    ...
    steps:
      ...
      - name: Store package.json as a result
        uses: actions/github-script@v3
        id: get_package_json
        with:
          script: core.setOutput('package_json', require('fs').readFileSync('package.json', 'utf8'))
      ...
      - name: Extract node_modules/.package.json
        run: git cat-file blob "FETCH_HEAD:package.json" > node_modules-package.json

      - name: Check package.json vs. node_modules/.package.json consistency
        run: diff -u node_modules-package.json package.json

  rebuild:
    needs: check
    if: ${{ needs.check.result == 'failure' && needs.check.outputs.package_json != '' }}
    runs-on: 'ubuntu-latest'
    steps:
      - name: debug
